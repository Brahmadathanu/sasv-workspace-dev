/**
 * UI-MOD-3E — MCM presentation remaps after CSS span restore.
 * Presentation only. Does not alter DOM IDs or workflows.
 */
import fs from "fs";
...
h = h.replace(/<body([^>]*)>/i, (full, attrs) => {
  ...
  if (/class="/i.test(a)) {
    a = a.replace(/class="([^"]*)"/i, (m, cls) => {
      ...
      parts.add("sasv-material-cost-manager");
      return `class="${[...parts].join(" ")}"`;
    });
  } else {
    a = ` class="sasv-module sasv-module--app sasv-costing sasv-material-cost-manager"${a}`;
  }
  return `<body${a}>`;
});
...
for (const [from, to] of replacements) {
  if (!h.includes(from)) {
    console.warn("MISSING block, skip:\n", from.slice(0, 80));
    continue;
  }
  h = h.replace(from, to);
}

// Post-restore legacy blues / lanes / chips / table-card (same patterns as CCC)
const blueMaps = [
  [
...
// peq filter svg 15 -> 16 (first occurrence in filter btn)
h = h.replace(
...
console.log(
  JSON.stringify(
    {
      body: (h.match(/<body[^>]*>/) || [""])[0].replace(/\s+/g, " ").slice(0, 260),
      ...
      pageRoot: /sasv-material-cost-manager/.test(h),
    },
    null,
    2
  )
