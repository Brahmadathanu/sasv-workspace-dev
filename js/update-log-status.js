...
async function configureDoneModal(activity, item, batch, id) {
  ...
  console.debug("configureDoneModal", {
    activity,
    act,
    id,
    skuActSetSize: skuActSet.size,
  });
  ...
  console.debug("configureDoneModal: rec.sku_breakdown", rec?.sku_breakdown);
  ...
  if (act === "finished goods quality assessment") {
    ...
    const { data: prod } = await supabase
      ...
      .single();
    if (prod) {
      // Load product SKUs and any existing packaging_event → event_skus
      const [{ data: skus }, { data: pe }] = await Promise.all([
        supabase
          .from("product_skus")
      ...
      console.debug("configureDoneModal: fetched skus/packaging_event", {
        skus,
        pe,
      });
      ...
      if (pe?.id) {
        ...
        if (exErr) console.error("event_skus lookup error:", exErr);
        existing = ex || [];
      }

      skus.forEach((s) => {
        const prev = existing.find((e) => e.sku_id === s.id);
        const valAttr = prev ? ` value="${prev.count}"` : "";
        doneSkuBody.insertAdjacentHTML(
          ...
          <tr>
            ...
            <td>${s.uom}</td>
            <td>
              <input type="number" min="0"${valAttr}
                     data-sku-id="${s.id}"
      ...
      console.debug("configureDoneModal: rendered skus", {
        rendered: skus.length,
        existing: existing.length,
      });

      // If there are no product SKUs, log a warning (server-side data likely missing/disabled)
      if (!skus.length) {
        console.warn(
...
async function promptDone(activity, item, batch, id) {
  ...
  return new Promise((res) => {
    ...
    const finish = async (choice) => {
      ...
      // 2) Lab Ref if shown
      if (
        doneLabRefSec.style.display !== "none" &&
        !document.querySelector("#doneLabRef").value.trim()
      ) {
      ...
      if (doneQtySection.style.display !== "none") {
        if (
          !document.querySelector("#doneQty").value ||
          !document.querySelector("#doneUOM").value
        ) {
          if (
            !(await askConfirm(
...
async function configureDoingModal(activity, item, batch, id) {
  ...
  const act = activity.toLowerCase().trim();
  doneJuiceSection.style.display = /juice|grinding|kashayam/.test(act)
    ? "flex"
    : "none";
  donePutamSection.style.display = /putam|gaja putam|seelamann/.test(act)
    ? "flex"
    : "none";
...
async function saveStatus(id, sel) {
  ...
  if (newStat === "Doing") {
    ...
    if (
      ...
      /putam|gaja putam|seelamann/.test(actL)
    ) {
