...
function populateCategory() {
  ...
  fillSelect(filterSubGroup, [], 'Subgroup');  filterSubGroup.disabled = true;
}
...
function populateItem() {
  ...
  filterBN.disabled = true; fillSelect(filterBN, [], 'BN');
}

function populateBN() {
  if (!filterItem.value) return;
  const bns = [...new Set(allRows.filter(r => r.item === filterItem.value).map(r => r.bn))]
...
function renderTable() {
  ...
  const order = [
    ['category',false],['product_group',false],['subgroup',false],
    ['subcategory',false],['item',false],['bn',true]
  ];
  rows.sort((a,b)=>{
    for(const [k,num] of order){
      const av=a[k]??'',bv=b[k]??'';
      if(av===bv) continue;
      return num ? av.localeCompare(bv,undefined,{numeric:true})
                 : (av<bv?-1:1);
    }return 0;
  });
...
function clearFilters() {
  [filterCategory,filterSubCat,filterGroup,filterSubGroup,
   filterItem,filterBN].forEach(el=>{
     el.value='';
...
/* ─── EXPORTS --------------------------------------------------- */
function exportCsv() {
  const hdr = ['ITEM','BN','QTY ON HAND','UOM','CATEGORY','SUBCATEGORY','PRODUCT GROUP','SUBGROUP'];
  const rows = [...tbody.rows]
    .filter(r=>!r.classList.contains('no-data'))
    .map(tr=>[...tr.cells].map(td=>`"${td.textContent.replace(/"/g,'""')}"`).join(','));
  const csv=[hdr.map(h=>`"${h}"`).join(','),...rows].join('\r\n');
  const blob=new Blob([csv],{type:'text/csv'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  a.href=url;a.download=`${todayStamp()}_fg_bulk_stock.csv`;
...
function exportPdf () {
  ...
  doc.setFont('Helvetica', 'bold').setFontSize(14)
     .text(`FG BULK STOCK AS ON ${new Date().toLocaleDateString('en-GB')}`,
...
window.addEventListener('DOMContentLoaded',async()=>{
  ...
  /* cascades */
  filterCategory.addEventListener('change',()=>{populateSubCategory();populateItem();renderTable();});
  filterSubCat.addEventListener('change',()=>{populateGroup();populateItem();renderTable();});
  filterGroup.addEventListener('change',()=>{populateSubGroup();populateItem();renderTable();});
  filterSubGroup.addEventListener('change',()=>{populateItem();renderTable();});
  filterItem.addEventListener('change',()=>{populateBN();renderTable();});
