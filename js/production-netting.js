...
function setDefaultWindow() {
  ...
  // inclusive last shown month = end-1 day (display only; queries use < end)
  document.getElementById("toDate").value = fmtDate(addMonths(end, -1));
}
...
async function loadGrid() {
  ...
  // derive [start, end) for DB filters using fence logic
  const start = fromISO || fmtDate(fenceStart());
  // toDate shown is inclusive last-month; compute exclusive end = (monthFloor(to)+1M)
  const toShown = toISO
  ...
  let q = supabase
    ...
    .select("product_id,month_start,net_bulk_to_make")
...
async function exportCSV() {
  ...
  while (true) {
    ...
    data.forEach((r) =>
      rows.push({
        ...
        net_bulk_to_make: r.net_bulk_to_make,
      })
    );
  ...
  a.href = url;
  a.download = `production_netting_${start}_to_${end}.csv`;
...
// ============= Drilldown (bottled & FG bulk by product) =============
  ...
  if (itemName) {
    ...
    (bot || []).forEach((r) => {
      ...
      tr.innerHTML = `
        ...
        <td>${r.on_hand}</td>
      `;
      bottledBody.appendChild(tr);
    });
  }

  // FG Bulk SOH (by batch) – filter by item name
  if (itemName) {
    const { data: fb, error: e2 } = await supabase
      .from("fg_bulk_stock")
      .select("bn, qty_on_hand, on_hand_qty_uom, last_updated, item")
      .eq("item", itemName)
...
function wire() {
  ...
  // modal controls
  document.getElementById("pdClose").addEventListener("click", closeDrill);
  document.getElementById("pdOk").addEventListener("click", closeDrill);
  const dlg = document.getElementById("pdModal");
