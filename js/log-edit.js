...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function populate(sel, rows, vKey, tKey, placeholder){
    ...
    (rows||[]).forEach(r=>{
      ...
      o.value = r[vKey]; o.textContent = r[tKey]; sel.append(o);
    });
  }

  // ─── Deletion cascade ──────────────────────────────────────────────────────
  async function deleteEntry(id){
    const { data:evts } = await supabase.from('packaging_events')
      .select('id').eq('work_log_id',id);
    const evtIds = (evts||[]).map(e=>e.id);
    if (evtIds.length){
      await supabase.from('event_skus').delete().in('packaging_event_id',evtIds);
      await supabase.from('packaging_events').delete().eq('work_log_id',id);
    }
    await supabase.from('daily_work_log').delete().eq('id',id);
  }

  // ─── Prompt Done (Qty/SKU/Transfer) ─────────────────────────────────────────
  async function promptDone(act,item,batch){
    ...
    if (skuActivities.includes(act)){
      ...
      let skus=[];
      if (prod){
        const { data } = await supabase.from('product_skus')
    ...
    return new Promise(res=>{
      ...
      doneJust.onclick  =()=>{
        ...
        hideModal(doneModal);
        res({choice:'just',rows,completedOn:doneCompletedOn.value,qty:+doneQty.value,uom:doneUOM.value});
      };
      doneNew.onclick   =()=>{
        let rows=null;
        if (skuActivities.includes(act)){
        ...
        hideModal(doneModal);
        res({choice:'new',rows,completedOn:doneCompletedOn.value,qty:+doneQty.value,uom:doneUOM.value});
      };
    });
  }
  ...
  async function initStatus(){
    ...
    const { data:secs } = await supabase.from('sections').select('id,section_name').order('section_name');
    ...
    sSection.onchange = async ()=>{
      if (!sSection.value) {
        ...
        const { data:subs } = await supabase.from('subsections')
          .select('id,subsection_name').eq('section_id',sSection.value).order('subsection_name');
        populate(sSub,subs,'id','subsection_name','Sub-section'); sSub.disabled=false;
      }
      populate(sArea,[], '', '','Area'); sArea.disabled=true;
      populate(sBN,  [], '', '','BN');   sBN.disabled=true;
      await loadStatus();
    };
    sSub.onchange = async ()=>{
      if (!sSub.value) {
        ...
        const { data:areas } = await supabase.from('areas')
          ...
          .order('area_name');
        populate(sArea,areas,'id','area_name','Area'); sArea.disabled=false;
      }
      populate(sBN,[], '', '','BN'); sBN.disabled=true;
      await loadStatus();
    };
    sArea.onchange = async ()=>{
      if (!sArea.value) {
        populate(sBN,[], '', '','BN'); sBN.disabled=true;
      } else {
        // load BNs via items filter or direct? Here just clear
        populate(sBN,[], '', '','BN'); sBN.disabled=true;
      }
      await loadStatus();
    };
    // Item→BN
    const { data:items } = await supabase.from('bmr_details').select('item',{distinct:true}).order('item');
    ...
    sItem.onchange = async ()=>{
      if (!sItem.value) {
        ...
        const uniqBns = [...new Set((bns||[]).map(r=>r.bn))].map(bn=>({bn}));
        populate(sBN,uniqBns,'bn','bn','BN');
        sBN.disabled=false;
      }
      await loadStatus();
    };
    ...
    document.querySelectorAll('.save-link').forEach(a=>{
      a.onclick = async e=>{
        ...
        if (ns==='Done'){
          ...
          if(result.choice==='cancel'){ sel.value='Doing'; return; }

          const upd = { status:'Done', completed_on:result.completedOn };
          if(skuActivities.includes(act)){
            upd.qty_after_process = null; upd.qty_uom = null;
            upd.sku_breakdown = result.rows.map(r=>`${r.sku_id} x ${r.count}`).join('; ');
          } else if(act==='transfer to fg store'){
            upd.qty_after_process = null; upd.qty_uom = null;
          } else {
            upd.qty_after_process = result.qty||null;
            upd.qty_uom           = result.uom;
          }
          ...
          if(skuActivities.includes(act)||act==='transfer to fg store'){
            ...
            const rows = (result.rows||[]).map(r=>({
              packaging_event_id:pe.id,sku_id:r.sku_id,count:r.count
            }));
            if(rows.length) await supabase.from('event_skus').insert(rows);
          }
          ...
          await supabase.from('daily_work_log')
  ...
  async function initFull(){
    ...
    fSection.onchange = async ()=>{
      if(!fSection.value){
        ...
        const { data:subs } = await supabase.from('subsections')
          .select('id,subsection_name').eq('section_id',fSection.value).order('subsection_name');
        populate(fSub,subs,'id','subsection_name','Sub-section'); fSub.disabled=false;
      }
    ...
    fSub.onchange = async ()=>{
      if(!fSub.value){
        ...
        const { data:areas } = await supabase.from('areas')
          ...
          .order('area_name');
        populate(fArea,areas,'id','area_name','Area'); fArea.disabled=false;
      }
      populate(fBN,[], '', '','BN'); fBN.disabled=true;
      loadFull();
    };
    ...
    fItem.onchange = async ()=>{
      if(!fItem.value){
        ...
        const { data:bns } = await supabase.from('bmr_details')
          .select('bn').eq('item',fItem.value).order('bn');
        const uniqBns = [...new Set((bns||[]).map(r=>r.bn))].map(bn=>({bn}));
        populate(fBN,uniqBns,'bn','bn','BN'); fBN.disabled=false;
      }
      loadFull();
    };
    clearFull.onclick = ()=>{
  ...
  async function openEditModal(id){
    const { data:[row] } = await supabase.from('daily_work_log')
      .select('*').eq('id',id).limit(1);
    if(!row) return;

    // Section→Sub→Area→Plant
    const { data:secs } = await supabase.from('sections').select('id,section_name').order('section_name');
    populate(e_section,secs,'id','section_name','Select Section');
    e_section.value = row.section_id;
    e_sub.disabled = e_area.disabled = e_plant.disabled = true;

    if(row.section_id){
      const { data:subs } = await supabase.from('subsections')
        .select('id,subsection_name').eq('section_id',row.section_id).order('subsection_name');
      populate(e_sub,subs,'id','subsection_name','Select Sub-section');
      e_sub.disabled=false; e_sub.value=row.subsection_id||'';
    }
    if(row.subsection_id){
      ...
      e_area.disabled=false; e_area.value=row.area_id||'';
    }
    if(row.area_id){
      const { data:plants } = await supabase.from('plant_machinery')
        .select('id,plant_name').eq('area_id',row.area_id).order('plant_name');
      populate(e_plant,plants,'id','plant_name','Select Plant');
      e_plant.disabled=false; e_plant.value=row.plant_id||'';
    }

    // Item→BN
    const { data:items } = await supabase.from('bmr_details')
      .select('item',{distinct:true}).order('item');
    populate(e_item,items,'item','item','Select Item');
    e_item.value = row.item;

    const { data:bns } = await supabase.from('bmr_details')
      .select('bn').eq('item',row.item).order('bn');
    const uniqBns = [...new Set((bns||[]).map(r=>r.bn))].map(bn=>({bn}));
    populate(e_bn,uniqBns,'bn','bn','Select Batch Number');
    e_bn.disabled=false; e_bn.value=row.batch_number;
    ...
    // Putam
    putamS.style.display = row.count_of_saravam!=null?'block':'none';
    ...
    if(row.status==='Done'){
      if(act==='finished goods quality assessment'){
        ...
        const { data:prod } = await supabase.from('products')
          .select('id').eq('item',row.item).single();
        let skus=[];
        if(prod){
          const { data } = await supabase.from('product_skus')
            .select('id,pack_size,uom')
        ...
        const { data:pe } = await supabase.from('packaging_events')
          .select('id').eq('work_log_id',id).single();
        let existing=[];
        if(pe){
          const { data } = await supabase.from('event_skus')
            .select('sku_id,count')
  ...
  editForm.onsubmit = async ev=>{
    ...
    if(upd.status==='Done' && (skuActivities.includes(act)||act==='transfer to fg store')){
      ...
      rows = rows.filter(r=>r.count>0).map(r=>({packaging_event_id:pe.id,...r}));
      if(rows.length) await supabase.from('event_skus').insert(rows);
    }
  ...
  modeStatus.onclick = ()=>{
    ...
    statusFilters.style.display='flex'; panelStatus.style.display='block';
    fullFilters.style.display='none'; panelFull.style.display='none';
    initStatus();
  };
  modeFull.onclick   = ()=>{
    modeFull.classList.add('active');
    modeStatus.classList.remove('active');
    fullFilters.style.display='flex'; panelFull.style.display='block';
    statusFilters.style.display='none'; panelStatus.style.display='none';
    initFull();
  };

  // get current user (unused here)
  supabase.auth.getUser();

  // start in Status mode
  modeStatus.click();
});