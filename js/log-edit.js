...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  editForm.onsubmit = async (ev) => {
    ...
    if (newStat === "On Hold") {
      ...
      if (errHold) {
        await showAlert(
          `Unexpected error: ${errHold.message || "see console"}`
        );
        return;
      }
      editSuccess.style.display = "block";
    ...
    if (newStat === "In Storage") {
      ...
      if (errStore) {
        ...
        await showAlert(
          `Error saving In Storage: ${
            errStore.message || errStore.details || "see console"
          }`
        );
        return;
      }
    ...
    if (updErr) {
      ...
      await showAlert(
        isDuplicateError(updErr)
          ...
          : `Unexpected error: ${
              updErr.message || updErr.details || "see console"
            }`
      );
      return;
    }
