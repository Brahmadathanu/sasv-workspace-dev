...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // packaging activities, including the two new “monocarton” variants
  const skuActivities = [
    'bottling',
  ...
  async function clearPackaging(logId) {
    const { data: evts } = await supabase
      .from('packaging_events').select('id').eq('work_log_id', logId);
  ...
  async function loadFull() {
    ...
    // join to plant_machinery to get plant_name
    let q = supabase
      .from('daily_work_log')
      .select(`
        id,
  ...
  async function initFull() {
    ...
    // 3) Cascading: Section → Sub-section
  ...
  async function saveStatusUpdate(id, sel) {
    ...
    if (newStat==='Done') {
      ...
      await supabase
        ...
        .update(upd)
        .eq('id', id);
      // 2) upsert packaging events if needed
  ...
  async function openEditModal(id) {
    ...
    if (row.section_id) {
      const { data: subs } = await supabase
        ...
        .eq('section_id', row.section_id).order('subsection_name');
    ...
    if (row.area_id) {
      const { data: plants } = await supabase
        ...
        .eq('area_id', row.area_id).order('plant_name');
    ...
    // dynamically adjust on status change
    e_status.onchange = () => {
      [editQtySection, labRefSection, editSkuSection, editTransSection]
        .forEach(sec => sec.style.display='none');
      editSkuBody.innerHTML = editTransBody.innerHTML = '';
  ...
  editForm.onsubmit = async ev => {
    ...
    // If user switches away from Done, clear all Done-data
    if (newStat !== 'Done') {
      await clearPackaging(id);
      await supabase
        .from('daily_work_log')
    ...
    // If Done, ensure qty/UOM if required
    ...
    if (actLower === 'finished goods quality assessment') {
      ...
      const container = skuActivities.includes(actLower) ? editSkuBody : editTransBody;
    ...
    // Upsert packaging events & event_skus if needed
