  ...
  // ────────────────────────────────────────────────────────────────────────────
    ...
    async function loadFull() {
      ...
      /* ──────────────────────────────
        0)  If “Residual bulk only” is
            ticked, fetch the ID list
      ────────────────────────────────*/
  ...
  if (fResidual.checked) {
    ...
    if (error) {
      ...
      // apply residual-batch filter (if present)
      if (residualMap) {
    q = q.in('id', Object.keys(residualMap).map(Number));
  }
    ...
    // ← compute this first, before using it
    ...
    <td class="actions-cell">
      <div class="dropdown">
        ...
        <ul class="dropdown-menu" role="menu">
          ...
          <li role="menuitem"><a href="#" class="zeroBtn"   data-id="${r.id}" data-act="${r.activity}" data-status="${r.status}">Zero&nbsp;&amp;&nbsp;Done</a></li>
        </ul>
      </div>
    </td>`;
      
  // hide / show the header immediately
  document.getElementById('thResidual').style.display = showResidual ? '' : 'none';
        fullBody.append(tr);
      });
      ...
      // Zero & Done → call fn_fg_force_close on the server
      document.querySelectorAll('.zeroBtn').forEach(btn => {
        btn.onclick = async e => {
          e.preventDefault();

      // guard: only allowed when row is In Storage AND activity = FG Bulk Storage
        const act    = (btn.dataset.act     || '').trim().toLowerCase();
        const status = (btn.dataset.status  || '').trim();
        if (act !== 'fg bulk storage' || status !== 'In Storage') {
          await showAlert('This action is only for FG Bulk Storage rows that are In Storage.');
          return;
        }
        if (!await askConfirm('Zero the residual stock and mark Done?')) return;
          ...
          if (error) {
            await showAlert(`Database refused: ${error.message || error.details}`);
            return;
          }

          await showAlert('Batch closed and residual stock adjusted to 0.');
          loadFull();        // refresh the table
        };
      });
