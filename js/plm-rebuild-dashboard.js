...
async function fetchOverview(horizonStart) {
  // Try each candidate view until one succeeds. This avoids hard-failing
  // when the expected view hasn't been created yet on the database.
  overviewRows = [];
  for (const candidate of PLM_OVERVIEW_VIEW_CANDIDATES) {
    try {
      ...
      if (error) {
        // If the relation doesn't exist (42P01) try the next candidate.
