...
async function fetchData() {
  const { data, error } = await supabase.from("bottled_stock_on_hand").select(`
      batch_number,
      pack_size,
      uom,
      on_hand,
...
function populateItem() {
  ...
  if (filterSubCat.value)
    rows = rows.filter((r) => r.sub_category === filterSubCat.value);
  if (filterGroup.value)
    rows = rows.filter((r) => r.group === filterGroup.value);
  if (filterSubGrp.value)
    rows = rows.filter((r) => r.sub_group === filterSubGrp.value);
...
function populateBN() {
  ...
  const bns = Array.from(new Set(rows.map((r) => r.batch_number))).sort(
...
async function exportPdf() {
  ...
  doc
    ...
    .setFontSize(14)
    .text(
      `BOTTLED STOCK AS ON ${new Date().toLocaleDateString("en-GB")}`,
      pw / 2,
      85,
      { align: "center" }
    );
