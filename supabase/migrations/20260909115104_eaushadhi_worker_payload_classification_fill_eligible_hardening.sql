-- Harden VERIFIED classification fill_eligible checks to fail closed.
-- Missing/null/false fill_eligible must reject; only exact text 'true' passes.
-- Does not rewrite 20260909112939. Payload shape and content_hash unchanged.
...
begin
  ...
  if v_class.review_status = 'VERIFIED' then
    ...
    -- Fail closed: only exact text 'true' is fill-eligible.
...
comment on function public.rpc_eaushadhi_worker_payload_get(integer, bigint) is
  'Read-only governed snapshot. Includes selected portal classification (never suggested/dossier fallback). VERIFIED options require source_context.fill_eligible exact text true. Classification participates in payload_hash.';
