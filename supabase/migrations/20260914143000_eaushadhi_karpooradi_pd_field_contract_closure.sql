-- Karpooradi Product Details field-contract closure.
-- Adds dossier portal shelfmonth route, extends payload_get details, reconciles content_get,
-- and adds narrow dossier portal-fields get/save RPCs with NOT_STARTED lifecycle guard.

begin;

alter table regulatory.product_dossier
  add column if not exists portal_shelfmonth_route text;

alter table regulatory.product_dossier
  drop constraint if exists product_dossier_portal_shelfmonth_route_check;

alter table regulatory.product_dossier
  add constraint product_dossier_portal_shelfmonth_route_check
  check (
    portal_shelfmonth_route is null
    or portal_shelfmonth_route in (
      'RegularAsPerClause',
      'Applyforaccessofshelflife'
    )
  );

-- payload_get: add dossier portal fields to details (full replace of latest body).
-- See 20260909115104 for prior history; this migration supersedes payload_get only.
...
begin
  ...
  if v_class.review_status = 'VERIFIED' then
    ...
    if (v_type.source_context->>'fill_eligible') is distinct from 'true' then
      raise exception using
        errcode = '22023',
    ...
    if (v_category.source_context->>'fill_eligible') is distinct from 'true' then
      raise exception using
        errcode = '22023',
    ...
    if v_mode = 'UNRESOLVED' then
      ...
      if (v_subtype.source_context->>'fill_eligible') is distinct from 'true' then
        raise exception using
          errcode = '22023',
  ...
  select jsonb_build_object(
    ...
    'details', jsonb_build_object(
      ...
      'portal_remarks', d.remarks,
      'portal_shelfmonth_route', d.portal_shelfmonth_route,
    ...
    'evidence', jsonb_build_object(
      'approved_product_copy_present', exists(
        select 1
        from regulatory.product_document doc
        where doc.product_id = p_product_id
          and doc.is_current = true
          and doc.document_purpose = 'APPROVED_PRODUCT_COPY'
      ),
...
comment on function public.rpc_eaushadhi_worker_payload_get(integer, bigint) is
  'Read-only governed snapshot. Includes dossier portal_remarks/portal_shelfmonth_route in details for worker field governance.';
...
begin
  ...
  v_hash := regulatory.eaushadhi_worker_content_hash(v_payload);
  return v_payload || jsonb_build_object('content_hash', v_hash);
end;
$function$;

comment on function public.rpc_eaushadhi_worker_content_get(integer, bigint) is
  'Read-only governed execution snapshot. Wraps payload_get and adds content_hash via eaushadhi_worker_content_hash.';

revoke all on function public.rpc_eaushadhi_worker_content_get(integer, bigint) from public;
revoke all on function public.rpc_eaushadhi_worker_content_get(integer, bigint) from anon;
grant execute on function public.rpc_eaushadhi_worker_content_get(integer, bigint) to authenticated, service_role;

create or replace function public.rpc_eaushadhi_product_dossier_portal_fields_get(
  p_product_id integer
)
returns jsonb
language plpgsql
...
as $function$
declare
  v_row regulatory.product_dossier%rowtype;
begin
  ...
  select * into v_row
  from regulatory.product_dossier d
  where d.product_id = p_product_id;

  if not found then
    raise exception using errcode = 'P0002', message = 'Product dossier not found';
  end if;

  return jsonb_build_object(
    'product_id', v_row.product_id,
    'remarks', v_row.remarks,
    'portal_shelfmonth_route', v_row.portal_shelfmonth_route,
    'row_version', v_row.row_version
  );
end;
$function$;

comment on function public.rpc_eaushadhi_product_dossier_portal_fields_get(integer) is
  'Read-only dossier portal fields for Review & Control (remarks + portal_shelfmonth_route).';

revoke all on function public.rpc_eaushadhi_product_dossier_portal_fields_get(integer) from public;
revoke all on function public.rpc_eaushadhi_product_dossier_portal_fields_get(integer) from anon;
grant execute on function public.rpc_eaushadhi_product_dossier_portal_fields_get(integer) to authenticated, service_role;

create or replace function public.rpc_eaushadhi_product_dossier_portal_fields_save(
  p_product_id integer,
  p_expected_row_version bigint,
  p_remarks text,
  p_portal_shelfmonth_route text
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_expected_row_version is null then
    raise exception using errcode = '22023', message = 'p_expected_row_version is required';
  end if;

  v_remarks := nullif(btrim(coalesce(p_remarks, '')), '');
  if v_remarks is null then
    raise exception using errcode = '22023', message = 'Portal remarks cannot be null or blank';
  end if;

  v_route := nullif(btrim(coalesce(p_portal_shelfmonth_route, '')), '');
  if v_route is null then
    raise exception using errcode = '22023', message = 'portal_shelfmonth_route is required';
  end if;

  if v_route not in ('RegularAsPerClause', 'Applyforaccessofshelflife') then
    raise exception using errcode = '22023', message = 'portal_shelfmonth_route is invalid';
  end if;
  ...
  if coalesce(v_workflow.entry_status, 'NOT_STARTED') is distinct from 'NOT_STARTED' then
    raise exception using
      errcode = '22023',
      message = format(
        'ENTRY_STATUS_NOT_EDITABLE: %s',
        coalesce(v_workflow.entry_status, 'NOT_STARTED')
      );
  end if;

  update regulatory.product_dossier d
  set
    remarks = v_remarks,
    portal_shelfmonth_route = v_route,
  ...
  if not found then
    raise exception using errcode = '40001', message = 'Stale dossier row version';
  end if;
...
comment on function public.rpc_eaushadhi_product_dossier_portal_fields_save(integer, bigint, text, text) is
  'Saves dossier portal remarks and shelfmonth route only while entry_status is NOT_STARTED.';

revoke all on function public.rpc_eaushadhi_product_dossier_portal_fields_save(integer, bigint, text, text) from public;
revoke all on function public.rpc_eaushadhi_product_dossier_portal_fields_save(integer, bigint, text, text) from anon;
grant execute on function public.rpc_eaushadhi_product_dossier_portal_fields_save(integer, bigint, text, text) to authenticated, service_role;
