-- e-Aushadhi browser-worker foundation: server-assembled preflight + payload.
-- Read-only. Does not update entry_status or workflow lifecycle columns.
-- Does not implement run_begin / ENTERED / PORTAL_VERIFIED / SUBMITTED.
...
begin
  ...
  if coalesce(v_workflow.entry_status, 'NOT_STARTED') = 'SUBMITTED' then
    v_reasons := array_append(v_reasons, 'Product is already SUBMITTED');
  end if;

  v_eligible :=
    v_has_readiness
    and coalesce(v_ready.is_ready_for_entry, false) is true
    and coalesce(v_workflow.entry_status, 'NOT_STARTED') is distinct from 'SUBMITTED';
...
comment on function public.rpc_eaushadhi_worker_preflight(integer) is
  'Read-only e-Aushadhi worker preflight. Does not change entry_status.';
comment on function public.rpc_eaushadhi_worker_payload_get(integer, bigint) is
  'Read-only governed worker payload snapshot with deterministic payload_hash.';
...
grant execute on function public.rpc_eaushadhi_worker_preflight(integer) to authenticated, anon, service_role;
grant execute on function public.rpc_eaushadhi_worker_payload_get(integer, bigint) to authenticated, anon, service_role;
