...
--
-- Company foundation (table/indexes/GET) is folded here with IF NOT EXISTS /
-- CREATE OR REPLACE so fresh bootstrap reaches a valid state and live-aligned
-- apply remains non-destructive. There is no invented "already applied"
-- parity migration version for these company objects.

-- ---------------------------------------------------------------------------
-- 0. Company sales-allocation default policy foundation (bootstrap-safe)
-- ---------------------------------------------------------------------------
...
comment on table costing.sales_allocation_default_policy is
  'Company CSA default units. Scenario-row units are authoritative after scenario-owned cutover.';

create or replace function public.rpc_get_sales_allocation_default_policies()
returns table (
  policy_id bigint,
  scenario_code text,
...
alter table costing.regional_sales_allocation_default_policy
  drop constraint if exists regional_sales_default_scenario_chk;

alter table costing.regional_sales_allocation_default_policy
  add constraint regional_sales_default_scenario_chk
  check (
    scenario_code = any (
      array[
...
-- ---------------------------------------------------------------------------
-- 10. Deprecate temporary shared fallback create (bootstrap-safe)
-- ---------------------------------------------------------------------------
do $deprecate$
begin
  if to_regclass('costing.sales_planning_fallback_unit_policy') is not null then
    execute $c$
  ...
  if to_regprocedure('costing.fn_resolve_sales_planning_fallback_units_as_of(date)') is not null then
    execute $c$
  ...
  if to_regprocedure(
    'costing.rpc_create_sales_planning_fallback_unit_policy(numeric,date,text,text)'
  ) is not null then
    execute $c$
...
-- ---------------------------------------------------------------------------
-- 11. Grants + internal mutation lockdown
-- ---------------------------------------------------------------------------
-- Public client surface (permission-checked inside each SECURITY DEFINER body):
grant execute on function public.rpc_get_sales_allocation_default_policies()
...
-- Read-only expander: intentionally callable by authenticated for diagnostics;
-- does not mutate and does not accept actor spoofing.
revoke all on function costing.fn_expand_sales_allocation_default_policy_scope(text, text, text, text)
  from public, anon;
grant execute on function costing.fn_expand_sales_allocation_default_policy_scope(text, text, text, text)
  to authenticated, service_role;

-- Mutation helpers: INTERNAL ONLY. Callable via security-definer public RPCs
-- owned by the migration role; must not be executable by ordinary clients.
revoke all on function costing.fn_revise_sales_allocation_default_policy(uuid, text, numeric, date, text, text)
  from public, anon, authenticated;
revoke all on function costing.fn_revise_regional_sales_allocation_default_policy(uuid, text, text, numeric, date, text, text)
  from public, anon, authenticated;

comment on function costing.fn_revise_sales_allocation_default_policy(uuid, text, numeric, date, text, text) is
  'INTERNAL mutation helper. EXECUTE revoked from PUBLIC/anon/authenticated. Call only via permission-checked public.rpc_set_* RPCs.';
comment on function costing.fn_revise_regional_sales_allocation_default_policy(uuid, text, text, numeric, date, text, text) is
  'INTERNAL mutation helper. EXECUTE revoked from PUBLIC/anon/authenticated. Call only via permission-checked public.rpc_set_* RPCs.';
